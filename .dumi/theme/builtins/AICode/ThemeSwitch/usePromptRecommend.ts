...
const fetchRecommendations = async (
  ...
  const options = {
    ...
    body: JSON.stringify({
        ...
        "description": "暗色主题，主色调蓝色，选中项加粗"
    }),
    // signal: abortSignal,
  };

  console.log('options===luokun===', options);

  try {
    const response = await fetch('http://localhost:3000/health', options);

    console.log('response===luokun===', response);
